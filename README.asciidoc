...
a new request would spin up a fresh server if it is down.
...
[source,kak]
----
hook -group lsp-filetype-clangd global WinSetOption filetype=(?:c|cpp) %{
...
See `kak-lsp --kakoune` for the default set of servers.
...
In `lsp_servers`, the `roots` parameter is a list of file globs.  Whenever your editor session
wants to send an LSP request, the first glob that matches a file in any of the current buffer's
parent directories will cause kakoune-lsp to set the project root to that parent directory.
...
During server initialization, kakoune-lsp sends the section specified by `settings_section`,
(which defaults to the server name) in this case `{"formatting.gofumpt":true}`.
Whenever you change the Kakoune option `lsp_config`, the same section is sent via
`workspace/didChangeConfiguration`.  Additionally, kakoune-lsp will send any sections that
...
Usually it is the same as Kakoune's filetype option but depending on the language server it may need to be different.
See `kak-lsp --kakoune` for the deviant cases.
...
* `lsp_snippet_support` toggles snippet support (completions with placeholders), see <<Snippets>>
* `lsp_file_watch_support` toggles file watch support, see <<Limitations>>;
...
The faces used for semantic tokens and modifiers can be modified by changing the `lsp_semantic_tokens` option, for example:
